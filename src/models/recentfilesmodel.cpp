...
// clang-format off
const QString OpenAction  = QStringLiteral("OpenAction");
const QString ExternalApp = QStringLiteral("ExternalApp");
    ...
    : QAbstractListModel(parent)
{
    connect(&youtube, &YouTube::videoInfoRetrieved, this, [this](YTVideoInfo info, QVariantMap data) {
        addRecentFile(info.url, data.value(QStringLiteral("openedFrom")).value<OpenedFrom>(), info.mediaTitle);
    });
    getItems();
}
...
void RecentFilesModel::addRecentFile(const QUrl &url, OpenedFrom openedFrom, const QString &name)
{
    ...
    if (url.scheme().startsWith(QStringLiteral("http")) && name.isEmpty()) {
        if (!url.toString().contains(QStringLiteral("list="))) {
            QVariantMap data{{QStringLiteral("openedFrom"), QVariant::fromValue(openedFrom)}};
