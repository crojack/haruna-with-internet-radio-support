    ...
    , m_mpv{static_cast<MpvItem *>(parent)}
{
    ...
    connect(PlaylistSettings::self(), &PlaylistSettings::PlaybackBehaviorChanged, this, [=]() {
        propertiesChanged(QStringLiteral("LoopStatus"), LoopStatus());
    });
    connect(PlaylistSettings::self(), &PlaylistSettings::RandomPlaybackChanged, this, [=]() {
        propertiesChanged(QStringLiteral("Shuffle"), Shuffle());
    });
}
...
QVariantMap MediaPlayer2Player::Metadata()
{
    ...
    if (mimeType.startsWith(QStringLiteral("audio"))) {
        ...
        m_metadata.insert(QStringLiteral("mpris:artUrl"), QString{});
    }
...
{
    if (loop == QStringLiteral("None")) {
        loop = QStringLiteral("StopAfterLast");
    } else if (loop == QStringLiteral("Track")) {
        loop = QStringLiteral("RepeatItem");
    } else if (loop == QStringLiteral("Playlist")) {
        loop = QStringLiteral("RepeatPlaylist");
    }
...
{
    if (PlaylistSettings::playbackBehavior() == QStringLiteral("RepeatItem")) {
        return QStringLiteral("Track");
    }
    if (PlaylistSettings::playbackBehavior() == QStringLiteral("RepeatPlaylist")) {
