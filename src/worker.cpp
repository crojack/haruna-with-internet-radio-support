...
void Worker::findRecursiveSubtitles(const QUrl &playingUrl)
{
    ...
    for (const auto &searchFolder : searchFolders) {
        ...
        while (it.hasNext()) {
            ...
            if (mimeType.startsWith(QStringLiteral("application/x-subrip")) || mimeType.startsWith(QStringLiteral("text/x-ssa"))) {
...
void Worker::getYtdlpVersion()
{
    ...
    ytdlpProcess.setArguments({QStringLiteral("--version")});
    ytdlpProcess.start();
...
QSqlDatabase Worker::getDBConnection()
{
    ...
    static auto db = QSqlDatabase::addDatabase(QStringLiteral("QSQLITE"), QStringLiteral("worker_connection"));
    if (!db.isOpen()) {
